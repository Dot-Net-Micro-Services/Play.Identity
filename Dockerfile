...
RUN --mount=type=secret,id=GH_OWNER,dst=/GH_OWNER --mount=type=secret,id=GH_PAT,dst=/GH_PAT \
    dotnet nuget add source --username USERNAME --password `cat GH_OWNER` --store-password-in-clear-text --name github "https://nuget.pkg.github.com/`cat GH_PAT`/index.json"
